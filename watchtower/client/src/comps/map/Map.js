...
function SimpleMap () {
  ...
  const nodePins = hubList ? hubList.map((hub) => {
    return (hub.nodes ? hub.nodes.map((node) => {
      return (
        <NodePin
          lat={node.lat}
          lng={node.long}
  ...
  const hubPins = hubList ? hubList.map((hub, index) => {
    return (
      <HubPin
        lat={hub.lat}
        lng={hub.long}
  ...
  return (
    ...
    <div style={{ height: '93vh', width: '100%' }}>
      <GoogleMapReact
        ...
        defaultCenter={{lat: 43.2608775, lng: -79.9214121}}
        defaultZoom={zoom}
        center={center}
      >
      {hubPins}
